...
\subsection{Resultados del primer ciclo}

Como ya se ha explicado anteriormente, durante el primer ciclo de pruebas lo realizó un solo usuario ya que en base a sus valoraciones se modificó la interfaz prácticamente por completo.
...
\begin{itemize}
	\item [ - ] Al girar el robot tal y como gira la cabeza, es muy difícil girar sobre uno mismo, y prácticamente imposible para una persona con discapacidad motora. 
	\subitem \textbf{Solución:} el robot gira con velocidad continua hacia la derecha o izquierda según la orientación de la cabeza del usuario.
	\item [ - ] Es poco intuitivo que la condición de parada sea mirar hacia un lado. Además es muy difícil de controlar que el robot esté avanzando por defecto.
	\subitem \textbf{Solución:} el robot por defecto está parado y en caso que esté en movimiento basta con mirar al frente para detenerlo.
\end{itemize}
...
Durante el segundo ciclo de pruebas participaron 
